...
/// data.
pub fn bar_plot_with_dropdown_for_different_data(_plot: &mut Plot) {
    ...
    _plot.set_layout(Layout::new().update_menus(vec![UpdateMenu::new().y(0.8).buttons(buttons)]));
}

/// Display a heat map, with buttons to allow for toggling of different
/// colorscales.
pub fn heat_map_with_modifiable_colorscale(_plot: &mut Plot) {
    ...
    _plot.set_layout(Layout::new().update_menus(vec![UpdateMenu::new()
...
/// display maodes.
pub fn bar_chart_with_modifiable_bar_mode(_plot: &mut Plot) {
    type BarType = Bar<&'static str, i32>;
    _plot.add_trace(
