...
fn load_apple_data() -> Vec<FinData> {
    ...
    println!("path:{:?}", p);
    p = p.join("src/app/assets").join("finance_charts_apple.csv");
...
pub fn time_series_plot_with_custom_date_range(_plot: &mut Plot) {
...
pub fn time_series_with_range_slider(_plot: &mut Plot) {
    let data = load_apple_data();
    let date: Vec<String> = data.iter().map(|d| d.date.clone()).collect();
    let high: Vec<f64> = data.iter().map(|d| d.high).collect();
...
pub fn time_series_with_range_selector_buttons(_plot: &mut Plot) {
    let data = load_apple_data();
    let date: Vec<String> = data.iter().map(|d| d.date.clone()).collect();
    let high: Vec<f64> = data.iter().map(|d| d.high).collect();
...
pub fn customizing_tick_label_formatting_by_zoom_level(_plot: &mut Plot) {
    let data = load_apple_data();
    let date: Vec<String> = data.iter().map(|d| d.date.clone()).collect();
    let high: Vec<f64> = data.iter().map(|d| d.high).collect();
...
// Candlestick Charts
pub fn simple_candlestick_chart(_plot: &mut Plot) {
    let x = vec![
        "2017-01-04",
        "2017-01-05",
        "2017-01-06",
...
// OHLC Charts
pub fn simple_ohlc_chart(_plot: &mut Plot) {
    let x = vec![
        "2017-01-04",
        "2017-01-05",
        "2017-01-06",
